...
This skill reads the canonical source Markdown files from `sources/` and the **ontology schema** from **`.cursor/rules/ontology.mdc`**, then generates or updates the derived Fumadocs content files under `content/`. It also ensures `content/meta.json` (and subfolder `meta.json` files) are valid for navigation.
...
- **Idempotency**: Re-running this skill will update existing `content/` files based on current `sources/` and the ontology rule; it will not create duplicate content.

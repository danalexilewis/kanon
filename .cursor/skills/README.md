...
|--------------------|--------|
| **`ingest/`**      | Process raw files from `ingest/` into `sources/` and `media/`; update manifests. |
| **`ingest-force/`**| Re-process all ingest files (idempotent). |
| **`create-ontology/`** | Build or refine the ontology schema in `.cursor/rules/ontology.mdc` from `references/` and user prompts. |
| **`update-docs/`**  | Generate/update `content/` from `sources/` and the ontology schema. |
